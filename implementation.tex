...
The click button on the GUI shown in fig 5.1 calls the createFile() function in class CreateInputFile, which is
responsible for file creation. Details for both the class are shown in fig 5.2.
...
SHA-3 hash function implementation is shown in fig 5.3.

\begin{figure}
  \begin{center}
    \includegraphics[width=6.8in]{SHA3classes.jpg}
  \end{center}
...
From the experiment selection dropdowns shown in fig 5.4, we can choose from the following factors
\begin{enumerate}
...
The classes that instantiate the experiment, and their relation with the GUI class is shown in fig 5.5. It is the
responsibility of the "Experiment" class, to go over the input files of the said category, and then provide the
collision finding algorithms with the input message pairs, and instantiating them.
...
UML class diagram shown in fig 5.6.
